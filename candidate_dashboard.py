...
# Contains the UI logic for the Candidate Dashboard
# =================================================================
...
def cv_management_tab_content():
    ...
    # (CV management form logic as provided in the original code)
    ...
    with st.form("cv_builder_form"):
        ...
        with col3:
            st.session_state.cv_form_data['phone'] = st.text_input(
                "Phone Number", value=st.session_state.cv_form_data['phone'], key="cv_phone"
            )
        
        col4, col5 = st.columns(2)
        with col4:
            st.session_state.cv_form_data['linkedin'] = st.text_input(
                "LinkedIn Profile URL", value=st.session_state.cv_form_data.get('linkedin', ''), key="cv_linkedin"
            )
        with col5:
            st.session_state.cv_form_data['github'] = st.text_input(
                "GitHub Profile URL", value=st.session_state.cv_form_data.get('github', ''), key="cv_github"
            )
        ...
        st.session_state.cv_form_data['personal_details'] = st.text_area(
            ...
            value=st.session_state.cv_form_data.get('personal_details', ''), height=100, key="cv_personal_details"
        )
        
        st.markdown("---")
        st.subheader("Technical Sections (One Item per Line)")

        def update_list_from_text_area(key, ss_key):
            current_list = st.session_state.cv_form_data.get(ss_key, [])
            text_value = "\n".join(current_list)
            new_text = st.text_area(key, value=text_value, height=150 if ss_key in ['skills', 'experience', 'projects'] else 100)
            st.session_state.cv_form_data[ss_key] = [s.strip() for s in new_text.split('\n') if s.strip()]

        update_list_from_text_area("Key Skills (Technical and Soft)", 'skills')
        update_list_from_text_area("Professional Experience (Job Roles, Companies, Dates, Key Responsibilities)", 'experience')
        update_list_from_text_area("Education (Degrees, Institutions, Dates)", 'education')
        update_list_from_text_area("Certifications (Name, Issuing Body, Date)", 'certifications')
        update_list_from_text_area("Projects (Name, Description, Technologies)", 'projects')
        update_list_from_text_area("Strengths / Key Personal Qualities (One per line)", 'strength')
